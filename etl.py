import json, requests, datetime, time, sys, subprocess
...
class SolrETL(object):
    ...
    def prepJson(self, json):
        ...
        self.adds += [dict(doc.items() + {'indexed' : { 'set' : timestamp } }.items()) for doc in json if u'id' in doc]
    ...
    def etlForPages(self):
        for host in self.hostsToIds.keys():
            ...
            subprocess.call('/usr/bin/python /usr/wikia/backend/bin/solr/crosswiki-reindex-harvester.py %s' % host+'/', shell=True)
        self.sendAddsAndDeletes()
